...
class LocalUpdate(object):
    ...
    def dp_sgd(self, model, global_round, norm_bound=1, noise_scale=1.0):
        #################
        ## ALGORITHM 1 ##
        #################
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                for i in range(self.args.local_bs):  #loss.size(0)
                    ...
                    for name, param in model.named_parameters():
                        ...
                        #param *= min(1, norm_bound / sum(param ** 2))
                        #per_sample_grad = param.grad.detach().clone()
                        #clip_grad_norm_(per_sample_grad, max_norm=norm_bound)
                ...
                # Add noise
                for param in model.parameters():
                    param += torch.normal(mean=0, std=noise_scale)
    ...
    def dp_sgdV2(self, model, global_round, norm_bound=1, noise_scale=1.0):
        #################
        ## ALGORITHM 1 ##
        #################
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                for i in range(self.args.local_bs):  # loss.size(0)
                    ...
                    # Clipped g_i
                    for param in model.parameters():
                        sum_clipped_grads += param * min(1, norm_bound / total_norm)
            ...
            for param in model.parameters():
                sum_clipped_grads += torch.normal(mean=0, std=noise_scale)
    ...
    def dp_sgdV3(self, model, global_round, norm_bound=15., noise_scale=10.0):
        #################
        ## ALGORITHM 1 ##
        #################
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                clip_factor = torch.clamp(g_norms ** 0.5 / norm_bound, min=1)  # potential for x/0 here...
                ...
                for param in model.parameters():
                    ...
                    #print(param.grad)
                    #print("mean")
                    #print(param.grad1.mean(dim=0))

                    # add noise
                    #param.grad.add_(torch.randn(param.size()) * 10 * noise_scale / len(labels))
                    ...
                    # update weights
                    #print(param.data)
                    param.data -= self.args.lr * (param.grad.data) # + torch.randn(param.size()) * noise_scale / len(labels))
                    #print(param.data)
